...
* is accessed both by scheduler callbacks and by print_gantt_line(). We
* could use a simple mutex, and lock it while calling each callback,
* but then callback concurrency would not be exercised.
* We therefore intentionally let multiple callbacks run
 * simultaneously.
 *
* For the callback lock, the IRWL_WRITER should always be locked while
* a callback is executing on a CPU thread. The IRWL_READER should always be
 ...
 *
* There is one special case: idle. Idle is simulated by the scheduler,
 * not the library's.  So we simply set the state variable to CPU_IDLE, and
* call the scheduler callback.
 */
static void simulator_cpu_thread(unsigned int cpu_id)
{
    simulator_cpu_state_t state;
    ...
    while (1)
    {
        ...
        case CPU_IDLE:
            /*
            * We can't lock the scheduler_lock for idle(); otherwise we can't
        ...
        case CPU_PREEMPT:
            IRWL_WRITER_LOCK(scheduler_lock)
            preempt(cpu_id);
            IRWL_WRITER_UNLOCK(scheduler_lock)
            break;
...
static void print_gantt_line(void)
{
     ...
     */
    IRWL_READER_LOCK(scheduler_lock)
    ...
    IRWL_READER_UNLOCK(scheduler_lock)
...
extern void force_preempt(unsigned int cpu_id)
{
    ...
    /*
    * It is possible that a scheduler callback calls force_preempt() at the
