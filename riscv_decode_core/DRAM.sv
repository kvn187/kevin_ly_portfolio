...
module DRAM (
  ...
  // ADDED to account for signed addresses / values
  logic signed [31:0] signed_data_addr;
  logic signed [31:0] mem_access_addr;

  assign signed_data_addr = $signed(data_addr_ip);
  ...
  always @(*) begin
    if (data_req_ip == 1'b1) begin
      ...
      case (lsu_operator)
        LW: begin
          load_data_op[31:24] = data_RAM[data_addr + signed_data_addr];
          load_data_op[23:16] = data_RAM[data_addr + signed_data_addr + 1];
          load_data_op[15:8] = data_RAM[data_addr + signed_data_addr + 2];
          load_data_op[7:0] = data_RAM[data_addr + signed_data_addr + 3];
        end
        default: 
  ...
  always @(posedge clock) begin
    if (data_req_ip == 1'b1) begin
      ...
      case (lsu_operator)
        SW: begin
          data_RAM[data_addr + signed_data_addr] = wdata_ip[31:24];
          data_RAM[data_addr + signed_data_addr + 1] = wdata_ip[23:16];
          data_RAM[data_addr + signed_data_addr + 2] = wdata_ip[15:8];
          data_RAM[data_addr + signed_data_addr + 3] = wdata_ip[7:0];
        end
      endcase
    end
  end
