...
module ID_Stage (
  ...
  */
  ...
  always @(*) begin
    alu_operator = ALU_NOP; // Default value
    ...
    en_lsu = 1'b0;
    lsu_operator = LW; // Default value. Fine as long as LSU is not enabled
  ...
  // Clear decode outputs when the pipeline is flushed.
  ...
  if (reset === 1'b1) begin
    ...
    end else if (stall_op === 1'b1) begin
      // Instructions to send to EX Stage
      alu_en_op <= 0;
      alu_operator_op <= ALU_NOP;
      alu_operand_a_ex_op <= 0;
